...
Não reimplementa a lógica de `executing-plan` — apenas remove os pontos de pausa que, no uso normal, esperariam confirmação do usuário, e cuida do commit/PR/saída do worktree ao final.
...
Uma vez que a PR é aberta, o trabalho já está preservado remotamente (branch publicada + PR), então o worktree local é descartado; ele só é preservado quando a PR não pôde ser aberta ou a execução foi interrompida antes disso.
...
**2. Commitar, abrir PR e sair.** Commite tudo dentro do worktree, publique a branch (`git push -u origin <branch>`) e abra um PR com `gh pr create`, com título e corpo que resumam a mudança e referenciem o plano. Com a PR aberta, use `ExitWorktree action: "remove"` para descartar o worktree local. Se não der para abrir PR (sem remoto, `gh` não autenticado, sem permissão de push), trate como limite de capacidade: mantenha as mudanças commitadas, saia com `ExitWorktree action: "keep"` (o worktree é a única cópia do trabalho) e relate isso na resposta.
...
- **Interrupção a pedido** — se o usuário mandar parar a qualquer momento, pare na hora e saia com `ExitWorktree action: "keep"` preservando o que já foi commitado.
